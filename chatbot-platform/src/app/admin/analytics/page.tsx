...
import ChatbotSelectionGrid from "@/components/admin-dashboard/analytics/ChatbotSelectionGrid";
...
export default function AdminAnalyticsPage() {
    ...
    // Get chatbotId from URL if present
    const searchParams = useSearchParams();
    const chatbotIdFromUrl = searchParams.get('chatbotId');
    
    // Use the same analytics hook as the client dashboard
    const { session, chatbotId, data, loading, error,
        timeRange, setTimeRange, refresh } = useAnalytics();
        
    // State for storing all chatbots
    const [allChatbots, setAllChatbots] = useState([]);
    const [chatbotsLoading, setChatbotsLoading] = useState(false);
    const [chatbotsError, setChatbotsError] = useState(null);
    
    // Fetch all chatbots for the selection screen
    useEffect(() => {
        // Only fetch if we're not looking at a specific chatbot
        if (!chatbotIdFromUrl && isAdmin) {
            const fetchChatbots = async () => {
                setChatbotsLoading(true);
                try {
                    const response = await fetch('/api/admin/chatbots');
                    if (!response.ok) {
                        throw new Error(`Error fetching chatbots: ${response.status}`);
                    }
                    const data = await response.json();
                    setAllChatbots(data);
                } catch (error) {
                    console.error("Failed to fetch chatbots:", error);
                    setChatbotsError(error.message);
                } finally {
    ...
    // If we don't have a specific chatbot ID, show the selection screen
    if (!chatbotIdFromUrl) {
        if (chatbotsLoading) return <LoadingSkeleton />;
                ...
                <div className="max-w-7xl mx-auto p-6">
                    <Card className="bg-[#1b2539] border-0">
                        <CardContent className="p-6 text-center">
                            <p className="text-red-400 mb-4">{chatbotsError}</p>
            ...
            <div className="max-w-7xl mx-auto p-6">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                    <h1 className="text-2xl font-bold">
                        Select a Chatbot for Analytics
                    </h1>
                    
                    <Link href="/admin/analytics/global" className="flex items-center text-blue-500 hover:text-blue-400">
                        View Global Analytics
                        <ArrowLeft className="h-4 w-4 ml-1 rotate-180" />
                    </Link>
                </div>
                
                <ChatbotSelectionGrid chatbots={allChatbots} />
            </div>
        );
    }

    // For specific chatbot view
        ...
        <div className="max-w-7xl mx-auto p-6">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                <div>
                    ...
                    <h1 className="text-2xl font-bold">
                        {data.chatbotName ? `Analytics: ${data.chatbotName}` : "Chatbot Analytics"}
                    </h1>
                </div>
                <TimeRangeSelector
                    value={timeRange}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                <EnhancedCartOperationsCard 
                    ...
                    extendedMetrics={true} // Always show extended metrics for consistency
